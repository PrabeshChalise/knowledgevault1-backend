...
export const listArtefacts = async (req, res) => {
  try {
    ...
    // Default: users are scoped to their own region.
    // Admin/reviewer can optionally query a specific region via ?regionId=...
    const effectiveRegionId =
      (req.user.roleGroup || "consultant") !== "consultant" && regionId ? regionId : req.user.regionId;

    const query = {
      regionId: new mongoose.Types.ObjectId(effectiveRegionId),
    };
    ...
    let mongoQuery = Artefact.find(query)
      ...
      .populate("reviewerDecision.reviewerId", "name email role");
...
export const getArtefactById = async (req, res) => {
  try {
    ...
    const isPrivileged = ["admin","reviewer"].includes(req.user.roleGroup || "consultant");
